...
if [[ -d "${SCRIPT_DIR}/cli" ]]; then
  ...
  PLUGIN_SRC_DIR="${SCRIPT_DIR}/cli/plugin"
else
...
# 修改安装路径到允许的目录
INSTALL_BASE="${SCRIPT_DIR}/.skillhub"
...
PLUGIN_TARGET_DIR="${SCRIPT_DIR}/.openclaw/extensions/skillhub"
...
BASE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/.skillhub"
...
#!/usr/bin/env bash
set -euo pipefail
exec "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/skillhub" "$@"
WRAPPER
...
if [[ "${MODE}" == "all" || "${MODE}" == "plugin" ]]; then
  install_plugin
  # 跳过配置插件步骤，因为无法在受限环境中修改用户的OpenClaw配置
  echo "Skipping plugin configuration due to permission restrictions."
fi
...
echo "Quick check:"
if [[ "${MODE}" == "all" || "${MODE}" == "cli" ]]; then
  echo "  ${WRAPPER_TARGET} search calendar"
fi
if [[ "${MODE}" == "all" || "${MODE}" == "plugin" ]]; then
