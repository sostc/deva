...
def main():
    ...
    # 更新执行代码
    print("更新执行代码...")
    result = quant_source.update_data_func_code(simple_gen_quant_code)
    ...
    if result.get("success"):
        ...
        try:
            # 编译测试
            local_vars = {}
            exec(simple_gen_quant_code, {"__builtins__": __builtins__}, local_vars)
            ...
            if "fetch_data" in local_vars and callable(local_vars["fetch_data"]):
                ...
                if test_result is not None:
                    ...
                    if hasattr(test_result, 'shape'):
                        ...
                        print(f"✅ 数据长度: {len(test_result) if hasattr(test_result, '__len__') else 1}")
                else:
