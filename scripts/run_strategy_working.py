...
class WorkingStockPickerSkill(StreamSkill):
    """可工作的选股策略 Skill"""
                    ...
                    if len(self.stock_history[code]) >= 2:
                        ...
                        if features:
                            ...
                            if score > buy_threshold:
                                candidates.append((code, score, name, price))
                        else:
                            # 有历史但无法计算特征，给默认分数
                            candidates.append((code, 0.55, name, price))
                
                # 买入得分最高的
                candidates.sort(key=lambda x: x[1], reverse=True)
                
                for code, score, name, price in candidates[:max_positions - len(self.positions)]:
                    buy_volume = 100
                    cost = price * buy_volume
                    
                    if cost > self.current_capital * 0.25:
                        continue
...
async def main():
    ...
    for config in strategies:
        ...
        engine.register_skill(skill_id, WorkingStockPickerSkill)
