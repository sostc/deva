...
def force_update_strategies():
    ...
    print(f"Found {len(entries)} strategies to update")
...
'''
    ...
    for entry in entries:
        print(f"\nUpdating strategy: {entry.name} (ID: {entry.id})")
        
        # 获取原始代码
        original_code = entry._func_code
        
        # 移除旧的过滤函数
        if "def is_valid_stock" in original_code:
            # 找到过滤函数的开始位置
            start_idx = original_code.find("import pandas as pd")
            # 找到过滤函数的结束位置
            end_idx = original_code.find("def process", start_idx)
            if end_idx != -1:
                # 移除旧的过滤函数
                original_code = original_code[end_idx:]
        
        # 在process函数开始处添加过滤逻辑
        if "def process(" in original_code:
            # 找到process函数的开始位置
            process_start = original_code.find("def process(")
            # 找到process函数的第一行代码
            first_line_end = original_code.find("\n", process_start)
            # 找到函数体的开始位置
            body_start = original_code.find("\n", first_line_end + 1)
            
            # 构建新的代码
            new_code = original_code[:body_start + 1]
            new_code += """
    # 过滤非个股数据
...
"""
            ...
            print(f"✗ No process function found, skipped")
            failed_count += 1
    
    print(f"\nSummary: Updated {updated_count} strategies, failed {failed_count}")


if __name__ == "__main__":
    force_update_strategies()
