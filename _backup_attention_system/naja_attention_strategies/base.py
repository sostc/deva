...
class AttentionStrategyBase(ABC):
    ...
    def should_execute(self, global_attention: Optional[float] = None) -> bool:
        """
        判断是否应该执行策略
        ...
        # 全局注意力低于阈值，跳过
        if global_attention < self.min_global_attention:
            self.skip_count += 1
            return False
    ...
    def process(self, data: pd.DataFrame, context: Optional[Dict[str, Any]] = None) -> List[Signal]:
        ...
        # 检查是否应该执行
        if not self.should_execute(global_attention):
            return []
