...
# 全局变量，用于跟踪 admin 是否正在启动或已启动
_admin_starting = False


def set_admin_starting():
    """设置 admin 正在启动的标志"""
    global _admin_starting
    _admin_starting = True
    
    # 为所有已创建的流对象启动 webview
    for name, stream in global_namespace.get('stream', {}).items():
        if hasattr(stream, 'webview'):
            try:
                stream.webview(f'/{hash(stream)}')
            except Exception:
                pass


def is_admin_starting():
    """检查 admin 是否正在启动或已启动"""
    global _admin_starting
    return _admin_starting
...
def NS(name='', *args, **kwargs):
    ...
    stream = global_namespace.create(typ='stream', name=name, *args, **kwargs)
    
    # 如果 admin 正在启动或已启动，则自动为流添加 webview
    if is_admin_starting() and hasattr(stream, 'webview'):
        try:
            stream.webview(f'/{hash(stream)}')
        except Exception:
            pass
