...
class DataSourceState(BaseState):
    ...
    def to_dict(self) -> dict:
        ...
        if self.last_update > 0:
            data["last_update_readable"] = datetime.fromtimestamp(self.last_update).isoformat()
        return data
...
class DataSource(StatusMixin, CallbackMixin):
    ...
    def record_data(self, data: Any):
        ...
        if hasattr(self._stream, 'emit'):
            try:
                # 始终使用守护线程中的IOLoop来执行emit操作，确保事件循环一致性
    ...
    def _save_running_state(self, is_running: bool):
        ...
        try:
            ...
            self.state.pid = os.getpid()  # 记录进程ID
    ...
    def _save_latest_data(self, data: Any):
        ...
        try:
            ...
            # 保存数据类型实例，用于策略引用
            self._save_data_type_instance(data)
        except Exception as e:
            self._log_event("ERROR", f"Failed to save latest data: {str(e)}")
    
    def _save_data_type_instance(self, data: Any):
        """保存数据类型实例，用于策略相关地方引用"""
        try:
            # 只保存非空数据
            if data is None:
            ...
            type_db = NB("data_source_type_instances")
            type_info = {
                "id": self._id,
                "name": self.name,
                "data_type": type(data).__name__,
                ...
                "type_class": type(data).__name__
            }
            type_db[f"{self._id}_type_instance"] = type_info
        except Exception as e:
            self._log_event("ERROR", f"Failed to save data type instance: {str(e)}")
    ...
    def _get_data_sample(self, data: Any) -> Any:
        ...
        try:
            ...
            if isinstance(data, pd.DataFrame):
                # 对于DataFrame，返回前2行作为样例
                return data.head(2).to_dict()
    ...
    def get_saved_running_state(self) -> Optional[dict]:
        ...
        try:
            # 从主表获取状态
            db = NB("data_sources")
            source_data = db.get(self._id)
            if source_data and "state" in source_data:
                state_data = source_data["state"]
                # 构建与之前格式兼容的状态信息
                return {
                    "id": self._id,
                    "name": self.name,
                    ...
                    "last_data_ts": state_data.get("last_data_ts", 0),
    ...
    def get_data_type_instance(self) -> Optional[dict]:
        """获取保存的数据类型实例，用于策略引用"""
        try:
            type_db = NB("data_source_type_instances")
            return type_db.get(f"{self._id}_type_instance")
        except Exception as e:
            self._log_event("ERROR", f"Failed to get data type instance: {str(e)}")
            return None
    
    def get_data_sample(self) -> Optional[Any]:
        """获取数据样例"""
        type_instance = self.get_data_type_instance()
        return type_instance.get("data_sample") if type_instance else None
    
    def get_data_type_info(self) -> Optional[dict]:
        """获取数据类型信息"""
        type_instance = self.get_data_type_instance()
        if type_instance:
            return {
                "data_type": type_instance.get("data_type"),
                "type_module": type_instance.get("type_module"),
                "type_class": type_instance.get("type_class"),
                "timestamp": type_instance.get("timestamp")
            }
        return None
...
class DataSourceManager(BaseManager[DataSource]):
    ...
    def create_source(
        ...
        try:
            ...
            register_result = self.register(source)
    ...
    def load_from_db(self) -> int:
        ...
        with self._items_lock:
            existing_names = {self._get_item_name(s) for s in self._items.values()}
            for source_id, data in all_items:
                if isinstance(data, dict):
                    try:
                        ...
                        if source.id not in self._items and self._get_item_name(source) not in existing_names:
                            self._items[source.id] = source
                            existing_names.add(self._get_item_name(source))
                            count += 1
...
def create_timer_source(
    ...
    register_result = ds_manager.register(source)
    if not register_result.get("success"):
        raise ValueError(f"Failed to create timer source: {register_result.get('error')}")
    return source
...
def create_stream_source(
    ...
    register_result = ds_manager.register(source)
    if not register_result.get("success"):
        raise ValueError(f"Failed to create stream source: {register_result.get('error')}")
    return source
...
def get_replay_tables() -> List[dict]:
    ...
    返回所有使用 key_mode='time' 创建且包含实际回放数据的表
    """
    from deva import NB
    ...
    try:
        ...
        for table in tables:
            ...
            try:
                ...
                # 尝试获取最近的时间范围的键，检查是否有实际的时间戳键
                # 使用一个合理的时间范围，确保能覆盖实际数据
                import time
                ...
                keys = list(test_stream[start_time:end_time])
                
                # 只有当表中存在时间戳键时，才认为支持回放
                if keys:
...
def create_replay_source(
    ...
    register_result = ds_manager.register(source)
    if not register_result.get("success"):
        raise ValueError(f"Failed to create replay source: {register_result.get('error')}")
    return source
...
def _init_quant_source():
    """初始化行情数据源
    
    确保quant_source和quant变量被正确初始化
    """
    global quant_source, quant
    
    # 尝试获取已存在的quant_source
