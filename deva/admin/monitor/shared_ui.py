"""Shared monitor UI implementation."""
...
def _ensure_stream_webview(ctx, stream, stream_id: str):
    if hasattr(stream, "webview"):
        try:
            stream.webview(f"/{stream_id}")
        except Exception as e:
            f"流 {stream_id} webview执行失败: {str(e)}" >> ctx["log"]
    else:
        f"流 {stream_id} 没有webview方法" >> ctx["log"]
...
async def view_stream(ctx, stream_id):
    ...
    _ensure_stream_webview(ctx, stream, stream_id)
