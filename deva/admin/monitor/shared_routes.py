"""Shared monitor route handlers used by both legacy and package paths."""
...
def _resolve_template_dir(module_file: str) -> str:
    module_path = Path(module_file).resolve()
    for parent in (module_path.parent, *module_path.parents):
        candidate = parent / "page_ui" / "templates"
        if candidate.exists():
            return str(candidate)
    # Keep a deterministic fallback path even when templates are missing.
    return os.path.join(os.path.dirname(os.path.dirname(module_file)), "page_ui", "templates")
...
def _render_stream_page(handler, stream, streams_template):
    handler.set_header("Content-Type", "text/html; charset=utf-8")
    handler.finish(streams_template.generate(streams=[stream], sockjs_prefix=""))
...
def build_monitor_route_handlers(ctx, *, module_file: str):
    """Build monitor-compatible handlers for admin tornado app."""
    template_dir = _resolve_template_dir(module_file)
    streams_template = Loader(template_dir).load("streams.html")
    route_ctx = {
        "Stream": ctx["Stream"],
        "log": ctx["log"],
        "global_ns": ctx,
    ...
    class MonitorLegacyStreamIdHandler(RequestHandler):
        ...
        def get(self, stream_id):
            ...
            _render_stream_page(self, stream, streams_template)
...
__all__ = ["build_monitor_route_handlers"]
