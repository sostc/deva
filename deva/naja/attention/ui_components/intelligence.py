...
def render_sector_predictive_attention_panel() -> str:
    """渲染预测板块注意力面板"""
    intelligence_system = get_intelligence_system()
    if not intelligence_system or not hasattr(intelligence_system, 'predictive_engine'):
        return """
        <div style="background: #fef3c7; border: 1px solid #fcd34d44; border-radius: 12px; padding: 16px;">
            <div style="font-weight: 600; color: #64748b;">🏛️ 预测板块 <span style="font-size: 11px; color: #94a3b8;">未启用</span></div>
    ...
    try:
        ...
        top_sector_predictions = intelligence_system.predictive_engine.get_sector_predictions_top_k(k=20)
        filtered_predictions = [
            (sector_id, score) for sector_id, score in top_sector_predictions
            if not is_noise_sector(sector_id)
        ][:5]

        noise_count = len(top_sector_predictions) - len(filtered_predictions)

        if not filtered_predictions:
            return """
            <div style="background: #fef3c7; border: 1px solid #fcd34d44; border-radius: 12px; padding: 16px;">
                <div style="font-weight: 600;">🏛️ 预测板块</div>
                <div style="color: #64748b; font-size: 12px; margin-top: 8px;">暂无有效板块预测数据（已过滤噪声板块）</div>
            </div>
            """
        ...
        <div style="margin-top: 12px; padding-top: 12px; border-top: 1px dashed #fcd34d; font-size: 11px; color: #64748b;">
            💡 板块预测 = 板块内个股加权扩散概率，高分=板块活跃可能持续 | 已过滤 {noise_count} 个噪声板块
        </div>
        """
        ...
        <div style="background: #fef3c7; border: 1px solid #fcd34d44; border-radius: 12px; padding: 16px;">
            <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;">
                ...
                <span style="font-weight: 600;">预测板块</span>
            </div>
            {time_html}
            {items_html}
            {help_text}
...
def render_propagation_panel() -> str:
    ...
    try:
        ...
        for rel in relations:
            if hasattr(rel, 'source_sector') and hasattr(rel, 'target_sector'):
                if rel.source_sector not in blacklist and rel.target_sector not in blacklist:
        ...
        if not filtered_relations:
            ...
            <div style="color: #94a3b8; font-size: 12px;">
                <div>正在学习有效板块关联...</div>
                <div style="margin-top: 4px; font-size: 11px;">已过滤无意义的板块关系</div>
            </div>
            """
        else:
            status_html = f"""
            <div style="font-size: 11px; color: #64748b; margin-bottom: 8px;">
                共 {summary.get('total_sectors', 0)} 个板块，已过滤 {len(blacklist)} 个噪声板块
            </div>
...
def render_sector_propagation_panel() -> str:
    ...
    try:
        ...
        valid_sectors = []
        if tracker:
            valid_sectors = tracker.filter_valid_sectors(list(blacklist))
        ...
        <div style="background: #fef3c7; border: 1px solid #fcd34d; border-radius: 12px; padding: 12px; margin-top: 12px;">
            <div style="font-weight: 600; margin-bottom: 8px; color: #92400e;">🚫 噪声板块黑名单 ({len(blacklist)} 个)</div>
            <div style="font-size: 11px; color: #78350f;">
                已自动过滤: 含B股、通达信88等噪声板块
            </div>
        </div>
        """
    except Exception:
