...
def _build_attention_flow_html(stats: Dict, context: Dict, integration, is_cn: bool, is_us: bool, us_data: Dict) -> str:
    ...
    if integration and integration.attention_system:
        try:
            attn_engine = integration.attention_system.global_attention
    ...
    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 4px; margin-bottom: 6px;">
        ...
        <div style="background: rgba(251,146,60,0.1); padding: 6px; border-radius: 4px; text-align: center;">
            ...
            <div style="font-size: 8px; color: #64748b;">活跃板块</div>
        </div>
        <div style="background: rgba(248,113,113,0.1); padding: 6px; border-radius: 4px; text-align: center;">
            <div style="font-size: 14px; font-weight: 700; color: #f87171;">{high_attention_symbol_count}</div>
            <div style="font-size: 8px; color: #64748b;">高注意股</div>
        </div>
    </div>
    <div style="padding: 4px 6px; background: rgba(74,222,128,0.05); border-radius: 4px; border-left: 2px solid {attention_color};">
        <div style="font-size: 9px; color: {attention_color}; font-weight: 600;">
            {'🔥 活跃' if global_attention > 0.6 else ('📊 一般' if global_attention > 0.3 else '💤 冷清')}
        </div>
    </div>
    """
...
def render_noise_filter_panel() -> str:
    ...
    ">
        ...
        <div style="margin-bottom: 8px;">
            <div style="font-size: 8px; color: #a855f7; margin-bottom: 4px;">🔍 过滤模式 (板块名含以下关键词即被过滤)</div>
