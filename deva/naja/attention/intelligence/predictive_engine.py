...
class SecondOrderDifferentiator:
    ...
    def update(self, symbol: str, value: float, timestamp: float) -> float:
        ...
        if len(self._history[symbol]) < 3:
            return 0.5
        
        values = [v for v, _ in self._history[symbol]]
...
class MomentumPredictor:
    ...
    def update(
        ...
        if len(self._momentum_history[symbol]) < 3:
            return 0.5
        
        momentums = list(self._momentum_history[symbol])
...
class PredictiveAttentionEngine:
    ...
    def _calc_prediction_score(
        ...
        if self.ema and returns is not None:
            _, _, ema_score = self.ema.update(symbol, returns, timestamp)
            scores.append(ema_score)
            weights.append(0.3)
        
        if self.diff and returns is not None:
            diff_score = self.diff.update(symbol, returns, timestamp)
