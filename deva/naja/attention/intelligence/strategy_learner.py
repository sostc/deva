...
class MarketState:
    ...
    avg_sector_attention: float
    max_sector_attention: float
    ...
    def to_context(self) -> np.ndarray:
        ...
        return np.array([
            ...
            self.avg_sector_attention,
            self.max_sector_attention,
    ...
    def get_state_name(self) -> str:
        """获取状态名称"""
        if self.global_attention > 0.7:
            if self.volatility > 0.5:
                return "high_attention_high_volatility"
            else:
                return "high_attention_low_volatility"
        elif self.global_attention > 0.4:
...
class MarketStateDetector:
    ...
    def detect(
        ...
        """
        if sector_attention:
            avg_sector = np.mean(list(sector_attention.values()))
            max_sector = max(sector_attention.values())
        else:
            avg_sector = 0.0
            max_sector = 0.0
        ...
        volatility = abs(global_attention - avg_sector) if avg_sector > 0 else 0.0
        
        state = MarketState(
            global_attention=global_attention,
            avg_sector_attention=avg_sector,
            max_sector_attention=max_sector,
