...
实现 StrategyEntry 接口，代理到 AttentionStrategyManager
...
from .strategy_manager import get_strategy_manager, AttentionStrategyManager
...
ATTENTION_STRATEGY_DIAGRAM_INFO = {
    "global_sentinel": {
        ...
        "formula": "global_attention = Σ(symbol_weights) / n",
        "logic": [
            "接收全市场行情数据流",
            "计算全市场注意力加权总和",
            "判断市场整体活跃度等级",
        ...
        "output": "global_attention: 0.0-1.0, activity_level: cold/warm/hot, market_state",
        "principle": {
    ...
    "sector_rotation_hunter": {
        "icon": "🎯",
        "color": "#8b5cf6",
        "description": "板块轮动捕捉，追踪注意力在板块间的转移",
        "formula": "sector_momentum = Δ(attention_weight) / Δt",
        "logic": [
            "监测各板块注意力权重变化",
            "计算板块间轮动速度",
            "识别正在崛起/衰退的板块",
        ...
        "output": "rising_sectors: [...], falling_sectors: [...], rotation_speed",
        "principle": {
            "core_mechanism": "追踪注意力在板块间的流动方向和速度，识别正在崛起或衰退的板块。",
            "key_insights": [
                "资金总是从老热点流向新热点，轮动是市场的主题",
                "强势板块的回调往往是二次入场机会",
                "板块轮动速度反映了市场热点的切换频率"
            ],
            "when_to_use": "需要捕捉板块轮动机会、优化行业配置时使用。"
        }
    },
...
class AttentionStrategyWrapper(StrategyEntry):
    """
    注意力策略包装器

    将 AttentionStrategyBase 包装成 StrategyEntry 接口，
    使其可以接入策略管理系统进行统一管理。
    """
    ...
    def __init__(
        ...
        manager: Optional[AttentionStrategyManager] = None
    ):
        self._attention_strategy = strategy
        self._attention_manager = manager
    ...
    def _get_attention_manager(self) -> AttentionStrategyManager:
        if self._attention_manager is None:
            self._attention_manager = get_strategy_manager()
    ...
    def save(self) -> dict:
        return {"success": True, "message": "注意力策略由 AttentionStrategyManager 管理"}
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "AttentionStrategyWrapper":
        raise NotImplementedError("AttentionStrategyWrapper 不能从 dict 重建")
...
def wrap_attention_strategy(
    ...
    Returns:
        AttentionStrategyWrapper 包装实例
    """
    return AttentionStrategyWrapper(strategy, manager)


def register_attention_strategies_to_manager(
    strategy_mgr: "StrategyManager",
    attention_mgr: Optional[AttentionStrategyManager] = None
    ...
    if attention_mgr is None:
        attention_mgr = get_strategy_manager()

    wrapped_strategies = []

    for strategy_id, strategy in attention_mgr.strategies.items():
        wrapper = wrap_attention_strategy(strategy, attention_mgr)
