...
class BlockBanditSignalGenerator:
    ...
    def __init__(
        ...
        self._selector = BlockStockSelector()
        self._last_signal: Optional[Dict] = None
        self._positions: List[Dict] = []

    def on_data(self, data: Any) -> None:
        """处理数据"""
        self._selector.on_data(data)

        selector_signal = self._selector.get_signal()
        if not selector_signal:
            return

        selected = selector_signal.get("selected_stocks", [])
        if not selected:
            return

        positions = []
        for stock in selected[:self._max_positions]:
            confidence = stock.get("score", 0)
            ...
            position = {
                ...
                "amount": self._position_size * confidence,
                "reason": f"牛股精选，评分{confidence:.2f}",
            }
        ...
        if best:
            self._last_signal = {
                "signal_type": "bandit_signal",
                "score": best.get("score", 0),
                "direction": 1 if best.get("change", 0) > 0 else -1,
                "positions": positions,
    ...
    def _generate_html(self, signal: Dict) -> str:
        ...
        best = signal.get("best_position", {})

        if not positions:
            return '''
<div style="background:linear-gradient(135deg,#059669,#10b981);border-radius:16px;padding:16px;color:#fff;margin-bottom:12px;">
    <div style="font-weight:600;font-size:16px;margin-bottom:8px;">🎯 Bandit 交易信号</div>
    <div style="padding:20px;text-align:center;color:rgba(255,255,255,0.7);">暂无交易信号</div>
</div>
'''

        signal_type = best.get("signal_type", "BUY")
        signal_color = "#22c55e" if signal_type == "BUY" else "#ef4444"

        rows = []
        for pos in positions:
            change = pos.get("price", 0)
            conf = pos.get("confidence", 0)
            rows.append(f'''
<tr>
    <td style="padding:8px;">{pos.get("stock_code","")}</td>
    <td style="padding:8px;color:{signal_color};font-weight:600;">{signal_type}</td>
    <td style="padding:8px;">{conf:.0%}</td>
    <td style="padding:8px;">¥{pos.get("amount",0):,.0f}</td>
</tr>
''')

        html = f'''
<div style="background:linear-gradient(135deg,#059669,#10b981);border-radius:16px;padding:16px;color:#fff;margin-bottom:12px;">
    <div style="font-weight:600;font-size:16px;margin-bottom:8px;">🎯 Bandit 交易信号 ({signal_type})</div>

    <div style="background:rgba(255,255,255,0.1);border-radius:12px;padding:12px;margin-bottom:12px;">
        <div style="font-size:12px;opacity:0.7;">推荐标的</div>
        <div style="display:flex;justify-content:space-between;align-items:center;">
            <div>
                <div style="font-size:18px;font-weight:700;">{best.get("stock_code","")}</div>
                <div style="font-size:12px;opacity:0.7;">{str(best.get("stock_name",""))[:10]}</div>
            ...
            <div style="text-align:right;">
                <div style="font-size:20px;font-weight:700;color:{signal_color};">{signal_type}</div>
                <div style="font-size:12px;opacity:0.7;">置信度 {best.get("confidence",0):.0%}</div>
            </div>
        </div>
    </div>
    ...
    <table style="width:100%;font-size:12px;">
        <tr style="color:rgba(255,255,255,0.7);">
            ...
            <th style="padding:8px;text-align:center;">信号</th>
            <th style="padding:8px;text-align:center;">置信度</th>
...
'''
    ...
    def update_params(self, params: Dict) -> None:
        ...
        self._position_size = params.get("position_size", self._position_size)
...
SELECTOR_REGISTRY = {
    ...
    "block_bandit": BlockBanditSignalGenerator,
