...
class ConfigManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.config = HotspotStrategyConfig()
        self._load_config()
    ...
    def _create_default_config(self) -> HotspotStrategyConfig:
        """创建默认配置"""
        config = HotspotStrategyConfig()
        
        # 全局市场监控
        config.strategies['global_sentinel'] = StrategySettings(
            enabled=True,
            priority=10,
            min_global_hotspot=0.0,  # 始终运行
    ...
    def _dict_to_config(self, data: Dict) -> HotspotStrategyConfig:
        """字典转配置对象"""
        config = HotspotStrategyConfig(
        ...
        for strategy_id, settings_data in data.get('strategies', {}).items():
            config.strategies[strategy_id] = StrategySettings(
                ...
                min_global_hotspot=settings_data.get('min_global_hotspot', 0.0),
    ...
    def _config_to_dict(self, config: HotspotStrategyConfig) -> Dict:
        """配置对象转字典"""
        result = {
        ...
        for strategy_id, settings in config.strategies.items():
            result['strategies'][strategy_id] = {
                ...
                'min_global_hotspot': settings.min_global_hotspot,
