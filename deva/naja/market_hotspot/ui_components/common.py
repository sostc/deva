...
def get_hotspot_report() -> Dict[str, Any]:
    """获取市场热点系统报告"""
    integration = get_market_hotspot_integration()
    if integration:
        try:
            report = integration.get_hotspot_report()
            _lab_debug_log(f"get_hotspot_report: global_hotspot={report.get('global_hotspot', 0):.4f}, activity={report.get('activity', 0):.4f}")
            return report
        except Exception as e:
            _lab_debug_log(f"get_hotspot_report 异常: {e}")
            pass
    else:
        _lab_debug_log("get_hotspot_report: integration 为空")
    return {}
...
def is_hotspot_system_initialized():
    """检查市场热点系统是否已初始化"""
    integration = get_market_hotspot_integration()
    if integration is None:
        return False
    return integration.hotspot_system is not None
...
def initialize_hotspot_system():
    ...
    try:
        ...
        config = load_config()
        if config.enabled:
            hotspot_system = _init(config)
            toast("✅ 市场热点系统初始化成功！", color="success")
            run_js("setTimeout(() => window.location.reload(), 1000)")
        else:
            toast("⚠️ 市场热点系统被禁用", color="warning")
    except Exception as e:
        from pywebio.output import toast
