...
class BlockNoiseFilter:
    ...
    def is_noise(self, block_id: str, block_name: str = None) -> bool:
        ...
        if block_id in self._auto_blacklist:
            return True

        return False

    def add_to_auto_blacklist(self, block_id: str, reason: str = ""):
        """添加到自动黑名单"""
        self._auto_blacklist.add(block_id)
        log.debug(f"题材加入自动黑名单: {block_id} ({reason})")
...
class NoiseManager:
    ...
    def is_block_noise(self, block_id: str, block_name: str = None) -> bool:
        """判断题材是否为噪音"""
        return self._block_filter.is_noise(block_id, block_name)
...
def is_block_noise(block_id: str, block_name: str = None) -> bool:
    """快捷函数：判断题材是否为噪音"""
    return get_noise_manager().is_block_noise(block_id, block_name)
