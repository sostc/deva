"""
CognitiveSignalBus - 认知信号事件总线（人）

🧠 定位：天-地-人框架中的「人」
    - 「人」= 决策中枢，感知天和地的变化后做出决策
...
🔄 数据流：
    BlockNarrative（地）→ 发布 BLOCK_NARRATIVE_UPDATE
                              ...
                              CognitiveSignalBus
                                    ↓
    TimingNarrative（天）→ 发布 TIMING_NARRATIVE_UPDATE
                                    ↓
...
使用方式：
    ...
    # ManasEngine 订阅
    def on_cognitive_update(event):
        manas_engine.invalidate_cache()

    bus.subscribe("ManasEngine", on_cognitive_update)
"""
