...
def render_first_principles(ui):
    ...
    if mind:
        try:
            ...
            contradictions = contradiction_summary.get("contradictions_found", 0)
            total_insights = insights_summary.get("total_insights", 0)
            has_data = (nodes > 0 or contradictions > 0 or total_insights > 0)
        except Exception:
            pass
    ...
    depth_level = 0
    if depth:
        from deva.naja.cognition.analysis.first_principles_mind import ThoughtLevel
        depth_labels = {
        ...
        depth_label, depth_color, depth_level = depth_labels.get(depth, ("未知", "#94a3b8", 0))

    awareness_pct = 0
    awareness_color = "#94a3b8"
    awareness_label = "未就绪"
    if awareness:
    ...
    if not mind:
        ...
        <div style="text-align: center; padding: 24px 16px;">
            ...
            <div style="font-size: 12px; color: #64748b; margin-bottom: 8px;">推理引擎未就绪</div>
            <div style="font-size: 10px; color: #475569;">等待系统初始化，将在此展示因果推理和矛盾检测</div>
        </div>
        """)
        put_html('</div>')
        return
    ...
    if not has_data:
        ...
        <div style="text-align: center; padding: 24px 16px;">
            <div style="font-size: 32px; margin-bottom: 12px;">⏳</div>
            <div style="font-size: 12px; color: #64748b; margin-bottom: 8px;">正在构建认知模型</div>
            <div style="font-size: 10px; color: #475569;">系统正在学习市场因果关系，积累知识后将在此展示</div>
        </div>
        """)
        put_html('</div>')
        return
    ...
    put_html(f"""
        ...
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 6px; margin-bottom: 12px;">
            <div style="background: rgba(96,165,250,0.1); border: 1px solid rgba(96,165,250,0.2); padding: 5px 8px; border-radius: 6px; text-align: center;">
                <div style="font-size: 14px; color: #60a5fa; font-weight: 700;">{nodes}</div>
                <div style="font-size: 8px; color: #94a3b8;">节点</div>
            </div>
            <div style="background: rgba(96,165,250,0.1); border: 1px solid rgba(96,165,250,0.2); padding: 5px 8px; border-radius: 6px; text-align: center;">
                <div style="font-size: 14px; color: #60a5fa; font-weight: 700;">{edges}</div>
                <div style="font-size: 8px; color: #94a3b8;">边</div>
            </div>
            <div style="background: rgba(96,165,250,0.1); border: 1px solid rgba(96,165,250,0.2); padding: 5px 8px; border-radius: 6px; text-align: center;">
                <div style="font-size: 14px; color: #60a5fa; font-weight: 700;">{avg_degree:.1f}</div>
                <div style="font-size: 8px; color: #94a3b8;">平均度</div>
            </div>
            <div style="background: rgba(96,165,250,0.1); border: 1px solid rgba(96,165,250,0.2); padding: 5px 8px; border-radius: 6px; text-align: center;">
                <div style="font-size: 14px; color: #60a5fa; font-weight: 700;">{temporal}</div>
                <div style="font-size: 8px; color: #94a3b8;">时序模式</div>
            </div>
        </div>
    """)
    ...
    put_html(f"""
        ...
        <div style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 6px; margin-bottom: 12px;">
            <div style="background: rgba(34,197,94,0.08); border: 1px solid rgba(34,197,94,0.15); padding: 5px 8px; border-radius: 6px; text-align: center;">
                <div style="font-size: 13px; color: #22c55e; font-weight: 700;">{known_causes}</div>
                <div style="font-size: 8px; color: #94a3b8;">已知因</div>
            </div>
            <div style="background: rgba(34,197,94,0.08); border: 1px solid rgba(34,197,94,0.15); padding: 5px 8px; border-radius: 6px; text-align: center;">
                <div style="font-size: 13px; color: #22c55e; font-weight: 700;">{known_effects}</div>
                <div style="font-size: 8px; color: #94a3b8;">已知果</div>
            </div>
            <div style="background: rgba(34,197,94,0.08); border: 1px solid rgba(34,197,94,0.15); padding: 5px 8px; border-radius: 6px; text-align: center;">
                <div style="font-size: 13px; color: #22c55e; font-weight: 700;">{chains}</div>
                <div style="font-size: 8px; color: #94a3b8;">因果链</div>
            </div>
            <div style="background: rgba(34,197,94,0.08); border: 1px solid rgba(34,197,94,0.15); padding: 5px 8px; border-radius: 6px; text-align: center;">
                <div style="font-size: 13px; color: #22c55e; font-weight: 700;">{knowledge_size}</div>
                <div style="font-size: 8px; color: #94a3b8;">知识库</div>
            </div>
            <div style="background: rgba(34,197,94,0.08); border: 1px solid rgba(34,197,94,0.15); padding: 5px 8px; border-radius: 6px; text-align: center;">
                <div style="font-size: 13px; color: #22c55e; font-weight: 700;">{counterfactuals}</div>
                <div style="font-size: 8px; color: #94a3b8;">反事实</div>
            </div>
        </div>
    """)
    ...
    if total_insights > 0:
        ...
        type_badges = " ".join(
            f'<span style="display: inline-block; background: {c}25; color: {c}; padding: 2px 6px; border-radius: 4px; font-size: 9px; margin-right: 2px;">{name} {cnt}</span>'
