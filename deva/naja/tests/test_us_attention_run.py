...
# Step 1: Test sector mapping
print('\n[Step 1] US Stock Sector Mapping')
from deva.naja.attention.data.global_market_futures import US_STOCK_SECTORS, US_STOCK_CODES, US_SECTOR_LIST

print(f'   Stock count: {len(US_STOCK_CODES)}')
print(f'   Sector count: {len(US_SECTOR_LIST)}')
print(f'   Key stocks sectors:')
for sym in ['nvda', 'aapl', 'tsla', 'amd', 'baba', 'mstr']:
    print(f'     {sym}: {US_STOCK_SECTORS.get(sym, "Unknown")}')
...
print(f'   sector_attention:')
for sector, weight in sorted(result['sector_attention'].items(), key=lambda x: x[1], reverse=True):
...
print(f'   render_us_hot_sectors_and_stocks: {len(hot_html)} chars')
...
print(f'   get_us_attention_state keys: {list(state.keys())}')
